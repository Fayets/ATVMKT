...
async def auto_sync_reels() -> None:
    """Sincroniza reels de Instagram para todos los usuarios con conexión instagram"""
    try:
        with db_session:
            _ = db
            connections = list(ApiConnection.select().filter(lambda c: c.platform == "instagram"))
        ...
        for user_id in user_ids:
            try:
                result = await service.sync_instagram(user_id)
                print(f"[scheduler] Reels sync automático OK para {user_id}: {result}")
            except Exception as e:
                print(f"[scheduler] Reels sync automático FAILED para {user_id}: {e}")
    except Exception as e:
        print(f"[scheduler] Error general en auto_sync_reels: {e}")
